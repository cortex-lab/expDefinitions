function oriChoiceWorld(t, evts, p, vs, in, out, audio)
%% oriChoiceWorld
% Burgess 2AUFC task with orientation comparison
% 2017-06 Modified advancedChoiceWorld.m SF
...
oriLeft = p.targetOrientation(1);
oriRight = p.targetOrientation(2);
...
rewardToneSamples = p.rewardToneAmplitude*...
    mapn(rewardToneFreq, 0.1, audSampleRate, 0.02, nAudChannels, @aud.pureTone);
...
targetRight.orientation = oriRight;
targetRight.altitude = 0;
targetRight.sigma = [12,12];
targetRight.spatialFrequency = p.spatialFrequency;
targetRight.phase = phase;   %random phase
...
evts.targetOrientation = p.targetOrientation;
