...
phaseChangeFreq = paras.phaseChangeFrequency; % Hz

adverseStim = paras.adverseOrientation;
...
adverseDuration = cond(ori == adverseStim, stimulusDuration-rewardDelay, true, 0);

adverseOn = stimulusOn.delay(rewardDelay);

noiseBurstSamples = paras.noiseBurstAmp*...
    mapn(numAudChannels, adverseDuration*audSampleRate, @randn);

audio.noiseBurst = noiseBurstSamples.at(adverseOn); 
...
phaseChange = skipRepeats(floor(t*phaseChangeFreq)/phaseChangeFreq);

phase = 2*pi*phaseChange.map(@(v)rand);

% target = vis.grating(t, 'sinusoid', 'gaussian');
target = vis.grating(t, 'sinusoid', 'none');
...
events.adverseOrientation = adverseStim;
events.targetPhase = phase*stimulusOn.to(stimulusOff);
...
% function stimUpdates = randStimUpdate(tT, stimDur, numUpdates)
% 
% stimUpdates = tT + (stimDur-tT)*rand(numUpdates,1);
% 
% end
