function quiescentAudioVisWorld3(t, evts, p, vs, inputs, outputs, audio)
...
% audioSR = (192e3)*2; % fixed for now
audioSR = p.audSampleRate;      % EJ test 20/07/2016
audDev  = p.audDevIdx;
...
chan2Onset = onsetToneSamples.mapn(2,1,@repmat);
audio.onsetTone = chan2Onset.at(cueOn);         
...
noiseSamples = mapn(2,audioSR*p.noiseBurstDur, @randn);
audio.noiseBurst = at(p.noiseBurstAmp*noiseSamples.mapn(2,1,@repmat), feedback < 0);
...
chan2Pip = pipSamples.mapn(2,1,@repmat);
audio.pips = chan2Pip.keepWhen(stimPresent);
...
evts.endTrial = nextCondition.at(stimOff4);
