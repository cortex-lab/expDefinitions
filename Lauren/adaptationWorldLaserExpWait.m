function adaptationWorldLaserExpWait(t, evts, p, vs, in, out, audio)
%% adaptationWorld
...
interTrialAdaptTime = map(evts.newTrial, @(x) max([min([1 exprnd(.9,x,x)]) .5]));
adapterDuration = iff(evts.trialNum < 2, p.initialAdaptTime, interTrialAdaptTime); 
...
stimulusOn = sig.quiescenceWatch(adapterDuration, t, wheel, p.quiescThreshold);
%stimulusOn = adapterOff.delay(0.2);
interactiveDelay = map(evts.newTrial, @(x) max([min([1 exprnd(.9,x,x)]) .8]));
interactiveOn = sig.quiescenceWatch(interactiveDelay, t, wheel, p.quiescThreshold);
% interactiveDelay = 0;
% interactiveOn = stimulusOn.delay(interactiveDelay);
...
correctResponse = cond(contrastLeft > contrastRight, -1,... % contrast left
    contrastLeft < contrastRight, 1,... % contrast right /// (contrastLeft + contrastRight == 0), 0,... % no-go (zero contrast)
